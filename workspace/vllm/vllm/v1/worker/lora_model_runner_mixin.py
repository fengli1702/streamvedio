...
from vllm.v1.spec_decode.spec_lora_debug import spec_lora_log, LogType, get_next_step_id
...
class LoRAModelRunnerMixin:
    ...
    def _set_active_loras(
        ...
        # Log LoRA state for debugging
        active_lora_names = [req.lora_name for req in lora_requests]
        active_lora_ids = [req.lora_int_id for req in lora_requests]
        
        spec_lora_log({
            "type": LogType.LORA_STATE,
            "step": get_next_step_id(),
            "enable_lora": True,
            "num_active_loras": len(lora_requests),
            "active_lora_names": active_lora_names,
            "active_lora_ids": active_lora_ids,
            "token_lora_mapping_len": len(token_lora_mapping),
            "prompt_lora_mapping_len": len(prompt_lora_mapping),
        })
