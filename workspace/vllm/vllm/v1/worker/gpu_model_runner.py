...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # Speculative-decoding step telemetry carried from execute_model()
        # to sample_tokens() on the same runner step.
        self._last_spec_verify_time_ms: float | None = None
        self._last_spec_verify_tokens: float | None = None
    ...
    def execute_model(
        ...
        # Avoid leaking previous-step telemetry when this step exits early.
        self._last_spec_verify_time_ms = None
        self._last_spec_verify_tokens = None
        ...
        if self.speculative_config and get_pp_group().is_last_rank:
            self._last_spec_verify_time_ms = (time.perf_counter() - verify_t0) * 1000.0
            self._last_spec_verify_tokens = float(num_scheduled_tokens)
    ...
    def sample_tokens(
        ...
        def propose_draft_token_ids(sampled_token_ids):
            nonlocal spec_draft_time_ms, spec_draft_invocations
            ...
            spec_draft_time_ms += (time.perf_counter() - t0) * 1000.0
            spec_draft_invocations += 1
        ...
        # Record EAGLE acceptance statistics for debugging
        if self.speculative_config and self._draft_token_ids is not None:
            num_proposed = int(self._draft_token_ids.numel())
            num_accepted = int(len(valid_sampled_token_ids))
            record_eagle_accept(num_proposed, num_accepted)

        spec_step_metrics: dict[str, float] | None = None
        if self.speculative_config:
            num_proposed = 0
            if self._draft_token_ids is not None:
                if isinstance(self._draft_token_ids, torch.Tensor):
                    ...
                    num_proposed = int(sum(len(x) for x in self._draft_token_ids))
            num_accepted = int(len(valid_sampled_token_ids))
            num_rejected = max(0, num_proposed - num_accepted)
            accept_rate = (
            ...
            verify_time_ms = (
                float(self._last_spec_verify_time_ms)
                if self._last_spec_verify_time_ms is not None
                else float("nan")
            )
            verify_tokens = (
                float(self._last_spec_verify_tokens)
                if self._last_spec_verify_tokens is not None
                else float(num_accepted)
            )
            # In v1 runtime, rejected drafted tokens imply an extra verify bubble.
            reverify_count = 1.0 if num_rejected > 0 else 0.0
            early_stop_count = 1.0 if (num_proposed > 0 and num_accepted < num_proposed) else 0.0
            spec_step_metrics = {
                "accepted": float(num_accepted),
                "proposed": float(num_proposed),
                "acceptance_rate": float(accept_rate),
                "reject_tokens": float(num_rejected),
                "reverify_count": float(reverify_count),
                "early_stop_count": float(early_stop_count),
                "verify_tokens": float(verify_tokens),
                "draft_time_ms": float(spec_draft_time_ms),
                "verify_time_ms": float(verify_time_ms),
                "draft_invocations": float(spec_draft_invocations),
            }
        self._last_spec_verify_time_ms = None
        self._last_spec_verify_tokens = None
        ...
        with record_function_or_nullcontext("gpu_model_runner: ModelRunnerOutput"):
            output = ModelRunnerOutput(
                ...
                spec_step_metrics=spec_step_metrics,
            )
