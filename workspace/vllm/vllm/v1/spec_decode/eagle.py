...
class EagleProposer:
    ...
    def propose(
        ...
        spec_lora_log({
            ...
            "seq_lens_shape": list(common_attn_metadata.seq_lens.shape),
            "seq_lens_min": float(common_attn_metadata.seq_lens.min().item()),
            "seq_lens_max": float(common_attn_metadata.seq_lens.max().item()),
            "seq_lens_mean": float(common_attn_metadata.seq_lens.float().mean().item()),
        ...
        # Log EAGLE output state
        spec_lora_log({
            "type": LogType.EAGLE_OUTPUT,
            "step": step_id,
            "draft_shape": list(draft_token_ids.shape),
            "draft_dtype": str(draft_token_ids.dtype),
            "draft_numel": int(draft_token_ids.numel()),
            "draft_norm": float(draft_token_ids.float().norm().item()),
            "draft_max": int(draft_token_ids.max().item()),
            "draft_min": int(draft_token_ids.min().item()),
            "draft_mean": float(draft_token_ids.float().mean().item()),
        })
