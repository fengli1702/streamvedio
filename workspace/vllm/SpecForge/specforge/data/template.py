...
class ChatTemplate(BaseModel):
    ...
    # Optional: certain chat formats insert a custom span between assistant
    # and the next user turn (e.g., LLaMA-2's " </s><s>[INST] ").
    user_after_assistant_header: str | None = None
...
TEMPLATE_REGISTRY.register(
    ...
    template=ChatTemplate(
        ...
        user_after_assistant_header=" </s><s>[INST] ",
    ),
)

# Minimal template matching the identity tokenizer we ship for streaming models.
TEMPLATE_REGISTRY.register(
    name="llama2-lite",
    template=ChatTemplate(
