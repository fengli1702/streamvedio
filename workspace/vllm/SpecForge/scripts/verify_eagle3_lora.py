...
验证 EAGLE3 LoRA 训练是否成功。通过对比以下两个阶段来评估 LoRA 对模型行为的影响：
  B1 (base): EAGLE3 without LoRA
  B2 (lora): EAGLE3 with trained LoRA adapter
...
def prepare_random_prompts(num_prompts: int, seq_length: int, vocab_size: int = 32000) -> list[list[int]]:
...
def run_spec(
    ...
    *,
    ...
    with tempfile.NamedTemporaryFile(suffix=".jsonl", prefix=f"{tag}_draft_", delete=False) as tmp:
...
def compare_sequences(
    ...
    return {
        "num_prompts": prompts,
        "valid_prompts": valid,
        "token_match_rate": (match_positions / total_positions) if total_positions else 0.0,
...
def main() -> None:
    ...
    parser.add_argument("--draft-model-base", type=Path, required=True,
                        help="基础 EAGLE3 draft 模型配置或路径")
    parser.add_argument("--lora-adapter-dir", type=Path, required=True,
                        help="训练好的 LoRA 适配器目录 (e.g., /workspace/SpecForge/output/lvm_eagle3_lora_v1/epoch_0_step_3475)")
    parser.add_argument("--num-prompts", type=int, default=32,
                        help="测试 prompt 数量")
    parser.add_argument("--prompt-length", type=int, default=512,
                        help="每个 prompt 的 token 长度")
    ...
    parser.add_argument("--draft-vocab-mapping-path", type=Path, default=None,
                        help="词表映射文件路径")
    parser.add_argument("--output", type=Path, default=Path("eagle3_lora_verification.json"))
    parser.add_argument("--save-sequences", action="store_true")
    parser.add_argument("--eagle-debug-dir", type=Path, default=None,
                        help="保存 EAGLE3 调试日志的目录")
    
    args = parser.parse_args()

    # 生成随机测试 prompts
    prompts = prepare_random_prompts(args.num_prompts, args.prompt_length, args.vocab_size)
    print(f"[INFO] 生成了 {len(prompts)} 个测试 prompts (长度: {args.prompt_length})")
    ...
    spec_cfg = {
        ...
        "model": str(args.draft_model_base),
    ...
    # B1: 基础 EAGLE3 (无 LoRA)
    print("\n" + "="*60)
    print("[PHASE] ===== EAGLE3 Base (without LoRA) =====")
    print("="*60)
    ...
    draft_base = run_spec("eagle3-base", base_args, prompts, sampling_params, None,
                          eagle_debug_dir=args.eagle_debug_dir)

    # B2: EAGLE3 with LoRA adapter
    print("\n" + "="*60)
    print("[PHASE] ===== EAGLE3 with LoRA Adapter =====")
    print("="*60)
    ...
    draft_lora = run_spec("eagle3-lora", lora_args, prompts, sampling_params, lora_request,
                          eagle_debug_dir=args.eagle_debug_dir)
    ...
    print(f"[METRIC] Exact Prefix Match Rate: {comparison['exact_prefix_match_rate']:.4f}")
    print(f"[METRIC] Valid Prompts: {comparison['valid_prompts']} / {comparison['num_prompts']}")
    ...
    if comparison['token_match_rate'] > 0.95:
        print("✅ [SUCCESS] LoRA adapter does NOT significantly affect EAGLE3 draft predictions!")
    else:
        print("⚠️  [WARNING] LoRA adapter may affect EAGLE3 draft predictions")
