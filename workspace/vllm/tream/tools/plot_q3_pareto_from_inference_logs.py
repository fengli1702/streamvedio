...
def read_metrics(path: str):
    # per-frame latency & accuracy from inference logs
    lats = []
    accs = []
    with open(path, "r") as f:
        for line in f:
            ...
            if not isinstance(obj, dict):
                continue
            if "latency" in obj and "accuracy" in obj:
...
def pchip_curve(xs: List[float], ys: List[float], points_per_seg: int = 30):
    if len(xs) < 2:
        return xs, ys
    x = np.array(xs, dtype=float)
    y = np.array(ys, dtype=float)

    # ensure strictly increasing x by keeping max y per x
    uniq = {}
...
def plot(rows, spec: str, out_path: str, ctx: int, inf: int, title_tag: str = ""):
    ...
    for mode in ["infer_only", "train_only", "joint"]:
        ...
        if not sub:
            continue
        xs = [r["lat_ms"] for r in sub]
        ys = [r["acc"] for r in sub]
        ax.scatter(xs, ys, c=colors[mode], marker=markers[mode],
...
def main():
    ...
    files = sorted(glob(os.path.join(args.logs_dir, "*.jsonl")))
    rows = []
    for fp in files:
    ...
    if not rows:
        raise SystemExit("No matching inference log rows")
    ...
    for spec in specs:
        ...
        if not sub:
            continue
        out = os.path.join(args.out_dir, f"q3_pareto_inferlog_spec{spec}_c{args.ctx}_i{args.inf}.png")
