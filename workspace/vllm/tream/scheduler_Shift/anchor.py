...
class AnchorTracker:
    ...
    def update(
        ...
        *,
        ...
        hold_confidence_gain: float = 0.08,
        hold_confidence_decay: float = 0.05,
    ) -> bool:
        promoted = False
        ...
        hold_confidence_gain = max(0.0, float(hold_confidence_gain))
        hold_confidence_decay = max(0.0, float(hold_confidence_decay))
        ...
        if self._same(decision.x_next, anchor):
            self.state.anchor_confidence = min(
                1.0, float(self.state.anchor_confidence) + hold_confidence_gain
            )
            self.state.trial = TrialState()
            self._update_anchor_margins(derived)
            return promoted
        ...
        if phase == Phase.HOLD_ANCHOR:
            ...
            self.state.anchor_confidence = max(
                0.0, float(self.state.anchor_confidence) - hold_confidence_decay
            )
            return promoted
