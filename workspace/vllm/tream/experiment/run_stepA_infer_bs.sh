...
RUN_PREFIX="${RUN_PREFIX:-stepA_bs}"
...
for infer_bs in ${INFER_BS_LIST}; do
  ...
  run_name="${RUN_PREFIX}_ctx${CTX_LEN}_inf${INF_LEN}_bs${infer_bs}_nospec_${sched_suffix}_data-${DATASET_TAG}"
  if [[ "${USE_SPEC}" == "1" ]]; then
    run_name="${RUN_PREFIX}_ctx${CTX_LEN}_inf${INF_LEN}_bs${infer_bs}_spec_${sched_suffix}_data-${DATASET_TAG}"
  fi
