...
RUN_SPEC="${RUN_SPEC:-1}"         # 1=on, 0=off
if [[ "${RUN_SPEC}" != "0" && "${RUN_SPEC}" != "1" ]]; then
  echo "RUN_SPEC must be 0 or 1, got: ${RUN_SPEC}" >&2
  exit 1
fi
...
if [[ "${RUN_MODE}" == "dynamic" ]]; then
  ...
  if [[ "${RUN_SPEC}" == "0" ]]; then
    MODE_TAG="static_nodyn_nospec_ib${IB}_tb${TB}"
  else
...
echo "[$(date '+%F %T')] MODE ${RUN_MODE} run_spec=${RUN_SPEC} mode_tag=${MODE_TAG}" | tee -a "${STATUS_LOG}"
...
JOBS=()
for i in "${!CASES_A[@]}"; do tag="$(printf "%02d" "$((i + 1))")"; JOBS+=("A${tag},${CASES_A[$i]}"); done
for i in "${!CASES_B[@]}"; do tag="$(printf "%02d" "$((i + 1))")"; JOBS+=("B${tag},${CASES_B[$i]}"); done
for i in "${!CASES_C[@]}"; do tag="$(printf "%02d" "$((i + 1))")"; JOBS+=("C${tag},${CASES_C[$i]}"); done
for i in "${!CASES_D[@]}"; do tag="$(printf "%02d" "$((i + 1))")"; JOBS+=("D${tag},${CASES_D[$i]}"); done
for i in "${!CASES_E[@]}"; do tag="$(printf "%02d" "$((i + 1))")"; JOBS+=("E${tag},${CASES_E[$i]}"); done
for i in "${!CASES_F[@]}"; do tag="$(printf "%02d" "$((i + 1))")"; JOBS+=("F${tag},${CASES_F[$i]}"); done

# Optional retry filters:
# - CASE_FILTER: comma-separated case ids, e.g. "A01,B04,C08"
# - SOURCE_RUN_ID: if set, auto-pick failed cases from ${LOG_DIR}/${SOURCE_RUN_ID}.tsv
#   by checking missing "${run_name}.jsonl".
CASE_FILTER="${CASE_FILTER:-}"
SOURCE_RUN_ID="${SOURCE_RUN_ID:-}"
declare -A WANT_CASE=()

if [[ -n "${CASE_FILTER}" ]]; then
  IFS=',' read -r -a _case_arr <<< "${CASE_FILTER}"
  for _case in "${_case_arr[@]}"; do
    [[ -n "${_case}" ]] && WANT_CASE["${_case}"]=1
  done
fi

if [[ -n "${SOURCE_RUN_ID}" ]]; then
  SOURCE_TSV="${LOG_DIR}/${SOURCE_RUN_ID}.tsv"
  if [[ ! -f "${SOURCE_TSV}" ]]; then
    echo "SOURCE_RUN_ID tsv not found: ${SOURCE_TSV}" >&2
    exit 1
  fi
  while IFS=$'\t' read -r _run_name _worker_lane _case_id _gpu _ray _ctx _inf _ib _tb _dlog _status; do
    [[ "${_case_id}" == "case_id" || -z "${_case_id}" ]] && continue
    if [[ ! -f "${LOG_DIR}/${_run_name}.jsonl" ]]; then
      WANT_CASE["${_case_id}"]=1
    fi
  done < "${SOURCE_TSV}"
fi
...
if (( ${#WANT_CASE[@]} > 0 )); then
  FILTERED_JOBS=()
  for _spec in "${JOBS[@]}"; do
    IFS=',' read -r _cid _ctx _inf <<< "${_spec}"
    if [[ -n "${WANT_CASE[${_cid}]:-}" ]]; then
      FILTERED_JOBS+=("${_spec}")
    fi
  done
  JOBS=("${FILTERED_JOBS[@]}")
...
if [[ -n "${CASE_FILTER}" ]]; then
  echo "[$(date '+%F %T')] FILTER case_filter=${CASE_FILTER}" | tee -a "${STATUS_LOG}"
fi
if [[ -n "${SOURCE_RUN_ID}" ]]; then
  echo "[$(date '+%F %T')] FILTER source_run_id=${SOURCE_RUN_ID}" | tee -a "${STATUS_LOG}"
fi

if (( TOTAL_CASES == 0 )); then
  echo "[$(date '+%F %T')] FINISH ${RUN_ID} no_cases_selected" | tee -a "${STATUS_LOG}"
  exit 0
fi
