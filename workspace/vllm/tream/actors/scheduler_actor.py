...
class SchedulerActor:
    def __init__(
        ...
        window_size: int = 30,
        context_bounds: Tuple[int, int] = (1, 8),
        inference_bounds: Tuple[int, int] = (1, 8),
        # qNEHVI defaults
        delta: float = 0.95,
        q_batch: int = 1,
        raw_samples: int = 64,
        ...
        cold_post_pause_windows = int(
            os.getenv("TREAM_SHIFT_COLD_POST_PAUSE_WINDOWS", "1")
        )
        ...
        anchor_confidence_gain = float(
            os.getenv("TREAM_SHIFT_ANCHOR_CONFIDENCE_GAIN", "0.25")
        )
        anchor_confidence_decay_hold_drift = float(
            os.getenv("TREAM_SHIFT_ANCHOR_CONFIDENCE_DECAY_HOLD_DRIFT", "0.05")
        )
        seek_exit_anchor_confidence = float(
            os.getenv("TREAM_SHIFT_SEEK_EXIT_ANCHOR_CONFIDENCE", "0.60")
        )
        seek_exit_min_probe_obs = int(
            os.getenv("TREAM_SHIFT_SEEK_EXIT_MIN_PROBE_OBS", "2")
        )
        seek_exit_stable_windows = int(
            os.getenv("TREAM_SHIFT_SEEK_EXIT_STABLE_WINDOWS", "2")
        )
        seek_exit_stable_radius = int(
            os.getenv("TREAM_SHIFT_SEEK_EXIT_STABLE_RADIUS", "0")
        )
        ...
        shock_disable_drift = os.getenv("TREAM_SHIFT_SHOCK_DISABLE_DRIFT", "0").strip().lower() not in (
            "0",
            "false",
            "no",
        )
        local_relax_enable = os.getenv("TREAM_SHIFT_LOCAL_RELAX_ENABLE", "1").strip().lower() not in (
            "0",
            "false",
            "no",
        )
        ...
        local_relax_q_slack = float(os.getenv("TREAM_SHIFT_LOCAL_RELAX_Q_SLACK", "0.05"))
        local_relax_lat_slack = float(os.getenv("TREAM_SHIFT_LOCAL_RELAX_LAT_SLACK", "0.08"))
        ...
        jsd_probe_up_max_step = int(os.getenv("TREAM_SHIFT_JSD_PROBE_UP_MAX_STEP", "1"))
        jsd_probe_down_max_step = int(os.getenv("TREAM_SHIFT_JSD_PROBE_DOWN_MAX_STEP", "2"))
        jsd_probe_down_step_mild = int(
            os.getenv("TREAM_SHIFT_JSD_PROBE_DOWN_STEP_MILD", "1")
        )
        jsd_probe_down_step_severe = int(
            os.getenv("TREAM_SHIFT_JSD_PROBE_DOWN_STEP_SEVERE", "2")
        )
        jsd_probe_force_refresh = os.getenv(
            "TREAM_SHIFT_JSD_PROBE_FORCE_REFRESH", "1"
        ).strip().lower() not in (
            "0",
            "false",
            "no",
        )
        jsd_probe_invalidate_radius = int(
            os.getenv("TREAM_SHIFT_JSD_PROBE_INVALIDATE_RADIUS", "3")
        )
        jsd_probe_invalidate_hold_ticks = int(
            os.getenv("TREAM_SHIFT_JSD_PROBE_INVALIDATE_HOLD_TICKS", "256")
        )
        jsd_probe_axis_order = str(
            os.getenv("TREAM_SHIFT_JSD_PROBE_AXIS_ORDER", "ctx,inf")
        )
        jsd_probe_continue_steps = int(
            os.getenv("TREAM_SHIFT_JSD_PROBE_CONTINUE_STEPS", "1")
        )
        ...
        if shock_disable_drift:
            # Disable token-shift/JSD driven ADAPT switching.
            shock_w_drift = 0.0
        ...
        if shift_quality_min is None and quality_min_default_raw:
            try:
                ...
                shift_quality_min = self.quality_min
        def _build_shift_scheduler_config(**kwargs: Any) -> SchedulerConfig:
            supported = set(SchedulerConfig.__dataclass_fields__.keys())
            filtered = {k: v for k, v in kwargs.items() if k in supported}
            dropped = sorted(k for k in kwargs.keys() if k not in supported)
            if dropped:
                print(
                    "[Scheduler] scheduler_Shift ignored unsupported config keys: "
                    + ",".join(dropped)
                )
            return SchedulerConfig(**filtered)

        self.shift_scheduler_config = _build_shift_scheduler_config(
            ...
            cold_start_post_pause_windows=max(0, int(cold_post_pause_windows)),
            ...
            anchor_confidence_gain=max(0.0, min(1.0, anchor_confidence_gain)),
            anchor_confidence_decay_hold_drift=max(
                0.0, min(1.0, anchor_confidence_decay_hold_drift)
            ),
            ...
            local_relax_quality_slack=max(0.0, float(local_relax_q_slack)),
            local_relax_latency_slack=max(0.0, float(local_relax_lat_slack)),
            jsd_local_probe_enable=bool(jsd_probe_enable),
            jsd_local_probe_up_max_step=max(1, int(jsd_probe_up_max_step)),
            jsd_local_probe_down_max_step=max(1, int(jsd_probe_down_max_step)),
            jsd_local_probe_down_step_mild=max(1, int(jsd_probe_down_step_mild)),
            jsd_local_probe_down_step_severe=max(1, int(jsd_probe_down_step_severe)),
            jsd_probe_force_refresh=bool(jsd_probe_force_refresh),
            jsd_probe_invalidate_radius=max(1, int(jsd_probe_invalidate_radius)),
            jsd_probe_invalidate_hold_ticks=max(1, int(jsd_probe_invalidate_hold_ticks)),
        ...
        cfg = self.shift_scheduler_config
        cfg_get = lambda name, default=None: getattr(cfg, name, default)
        print(
            ...
            f"cold_windows={cfg_get('cold_start_windows')}, "
            f"hold_radius={cfg_get('hold_anchor_radius')}, "
            f"reroute_radius={cfg_get('reroute_radius')}, "
            f"trial_promote={cfg_get('anchor_trial_promote_windows')}, "
            f"stabilizer={'on' if cfg_get('stabilizer_enable', False) else 'off'}, "
            f"stab_shadow={'on' if cfg_get('stabilizer_shadow', False) else 'off'}"
        )
        print(
            "[Scheduler] scheduler_Shift knobs "
            f"cold_max_windows={cfg_get('cold_start_max_windows')}, "
            f"cold_probe_every={cfg_get('cold_start_probe_every')}, "
            f"cold_probe_eps_fast={cfg_get('cold_start_probe_eps_fast')}, "
            f"cold_probe_eps_slow={cfg_get('cold_start_probe_eps_slow')}, "
            f"cold_relax_safety={cfg_get('cold_start_relax_safety')}, "
            f"cold_whitelist_enable={cfg_get('cold_whitelist_probe_enable')}, "
            f"cold_whitelist_budget={cfg_get('cold_whitelist_probe_budget')}, "
            f"min_count_for_trust={cfg_get('min_count_for_trust')}, "
            f"max_staleness_ticks={cfg_get('max_staleness_ticks')}, "
            f"seek_exit_conf={cfg_get('seek_exit_anchor_confidence')}, "
            f"seek_exit_min_probe_obs={cfg_get('seek_exit_min_probe_observations')}, "
            f"seek_exit_stable={cfg_get('seek_exit_stable_windows', 'na')}"
            f"@r{cfg_get('seek_exit_stable_radius', 'na')}, "
            f"cold_post_pause={cfg_get('cold_start_post_pause_windows', 'na')}, "
            f"anchor_bootstrap={cfg_get('anchor_bootstrap_windows')}, "
            f"anchor_init_conf={cfg_get('anchor_init_confidence')}, "
            f"anchor_conf_gain={cfg_get('anchor_confidence_gain', 'na')}, "
            ...
            f"(m{cfg_get('jsd_local_probe_down_step_mild', 1)}"
            f",s{cfg_get('jsd_local_probe_down_step_severe', 2)}), "
            f"jsd_refresh={cfg_get('jsd_probe_force_refresh', False)}"
            f"@r{cfg_get('jsd_probe_invalidate_radius', 3)}"
            f"/t{cfg_get('jsd_probe_invalidate_hold_ticks', 256)}"
            ...
            f"local_relax_drift_threshold={cfg_get('local_relax_drift_threshold', 0.0)}, "
    ...
    def _to_window_metrics(self, agg: Dict[str, Any]) -> WindowMetrics:
        ...
        return WindowMetrics(
            ...
            token_drift_mean=None if agg.get("token_drift_mean") is None else float(agg.get("token_drift_mean")),
    ...
    def _aggregate_metrics(self) -> Optional[Dict[str, Any]]:
        ...
        for m in self.inference_metrics:
            jsd_val = None
            for k in ("jsd", "drift_jsd", "jsd_mean", "token_drift_mean", "token_drift"):
                if k in m and m.get(k) is not None:
                    jsd_val = m.get(k)
                    break
        ...
        return {
            ...
            "token_drift_mean": _safe_mean(jsd_vals),
