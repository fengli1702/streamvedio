...
RUN_ID="abc6x3_trigpu_shift_taskpool_jsdonly_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_abc6x3_trigpu_shift_taskpool_jsdonly_run.txt"
...
echo "[$(date '+%F %T')] SHIFT_KNOBS jsd_only=${SHIFT_SHOCK_USE_JSD_ONLY} local_relax=${SHIFT_LOCAL_RELAX_ENABLE}/${SHIFT_LOCAL_RELAX_WINDOWS}/r${SHIFT_LOCAL_RELAX_RADIUS}/th${SHIFT_LOCAL_RELAX_DRIFT_THRESHOLD} cold_relax_safety=${SHIFT_COLD_RELAX_SAFETY} cold_windows=${SHIFT_COLD_START_WINDOWS} cold_max_windows=${SHIFT_COLD_START_MAX_WINDOWS}" | tee -a "${STATUS_LOG}"
...
# 6-point subset: small/medium/large + cross extremes.
JOBS=(
  "S01,1,1"
  "S02,4,4"
  "S03,1,8"
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_abc6x3_taskpool_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_abc6x3_${case_id}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
...
ray stop --force >> "${STATUS_LOG}" 2>&1 || true
start_ray_head "A" "${GPU_A}" "${RAY_PORT_A}" "${RAY_WORKER_MIN_A}" "${RAY_WORKER_MAX_A}" 54465 62221 59221 8565
start_ray_head "B" "${GPU_B}" "${RAY_PORT_B}" "${RAY_WORKER_MIN_B}" "${RAY_WORKER_MAX_B}" 54466 62222 59222 8566
start_ray_head "C" "${GPU_C}" "${RAY_PORT_C}" "${RAY_WORKER_MIN_C}" "${RAY_WORKER_MAX_C}" 54467 62223 59223 8567
