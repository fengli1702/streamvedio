...
RUN_ID="ab32x3_trigpu_shift_taskpool_${START_TS}"
LOG_DIR="/workspace/tream/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_ab32x3_trigpu_shift_taskpool_run.txt"
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_ab32x3_taskpool_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_ab32x3_${case_id}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
