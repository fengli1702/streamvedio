...
RUN_ID="ab24x2_dualgpu_shift_taskpool_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_ab24x2_dualgpu_shift_taskpool_run.txt"
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_ab24x2_taskpool_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_ab24x2_${case_id}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
