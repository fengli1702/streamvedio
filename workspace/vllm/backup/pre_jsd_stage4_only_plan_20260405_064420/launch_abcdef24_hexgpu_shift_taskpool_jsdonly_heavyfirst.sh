...
RUN_ID="abc24x6_hexgpu_shift_taskpool_jsdonly_heavyfirst_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_abc24x6_hexgpu_shift_taskpool_jsdonly_heavyfirst_run.txt"
...
RAY_PORT_A="${RAY_PORT_A:-29621}"
RAY_PORT_B="${RAY_PORT_B:-29622}"
RAY_PORT_C="${RAY_PORT_C:-29623}"
RAY_PORT_D="${RAY_PORT_D:-29624}"
RAY_PORT_E="${RAY_PORT_E:-29625}"
RAY_PORT_F="${RAY_PORT_F:-29626}"
...
RAY_ADDR_E="127.0.0.1:${RAY_PORT_E}"
RAY_ADDR_F="127.0.0.1:${RAY_PORT_F}"

RAY_WORKER_MIN_A="${RAY_WORKER_MIN_A:-42000}"
RAY_WORKER_MAX_A="${RAY_WORKER_MAX_A:-42999}"
RAY_WORKER_MIN_B="${RAY_WORKER_MIN_B:-43000}"
RAY_WORKER_MAX_B="${RAY_WORKER_MAX_B:-43999}"
RAY_WORKER_MIN_C="${RAY_WORKER_MIN_C:-44000}"
RAY_WORKER_MAX_C="${RAY_WORKER_MAX_C:-44999}"
RAY_WORKER_MIN_D="${RAY_WORKER_MIN_D:-45000}"
RAY_WORKER_MAX_D="${RAY_WORKER_MAX_D:-45999}"
RAY_WORKER_MIN_E="${RAY_WORKER_MIN_E:-46000}"
RAY_WORKER_MAX_E="${RAY_WORKER_MAX_E:-46999}"
RAY_WORKER_MIN_F="${RAY_WORKER_MIN_F:-47000}"
RAY_WORKER_MAX_F="${RAY_WORKER_MAX_F:-47999}"

SHIFT_WARMUP_HOLD_WINDOWS="${SHIFT_WARMUP_HOLD_WINDOWS:-2}"
SHIFT_WARMUP_EXPLORE_ENABLE="${SHIFT_WARMUP_EXPLORE_ENABLE:-0}"
SHIFT_WARMUP_FORCE_DESCEND_LARGE="${SHIFT_WARMUP_FORCE_DESCEND_LARGE:-1}"
SHIFT_WARMUP_LARGE_CTX_THRESHOLD="${SHIFT_WARMUP_LARGE_CTX_THRESHOLD:-6}"
SHIFT_WARMUP_LARGE_INF_THRESHOLD="${SHIFT_WARMUP_LARGE_INF_THRESHOLD:-6}"
SHIFT_WARMUP_OSC_EXIT_ENABLE="${SHIFT_WARMUP_OSC_EXIT_ENABLE:-1}"
SHIFT_WARMUP_OSC_PATIENCE="${SHIFT_WARMUP_OSC_PATIENCE:-4}"
SHIFT_WARMUP_OSC_UNIQUE_MAX="${SHIFT_WARMUP_OSC_UNIQUE_MAX:-2}"
...
echo "[$(date '+%F %T')] PLAN total_cases=24 dispatch=task_pool workers=6 pattern=heavyfirst_taskpool" | tee -a "${STATUS_LOG}"
echo "[$(date '+%F %T')] GPU_INIT A=${GPU_A} B=${GPU_B} C=${GPU_C} D=${GPU_D} E=${GPU_E} F=${GPU_F}" | tee -a "${STATUS_LOG}"
...
echo "[$(date '+%F %T')] SHIFT_FAST_CONVERGE warmup=${SHIFT_WARMUP_HOLD_WINDOWS} warmup_explore=${SHIFT_WARMUP_EXPLORE_ENABLE} descend_large=${SHIFT_WARMUP_FORCE_DESCEND_LARGE}@ctx${SHIFT_WARMUP_LARGE_CTX_THRESHOLD},inf${SHIFT_WARMUP_LARGE_INF_THRESHOLD} warmup_osc_exit=${SHIFT_WARMUP_OSC_EXIT_ENABLE}/${SHIFT_WARMUP_OSC_PATIENCE}/u${SHIFT_WARMUP_OSC_UNIQUE_MAX} probe_eps=${SHIFT_COLD_PROBE_EPS_FAST} whitelist=${SHIFT_COLD_WHITELIST_BUDGET}@lat${SHIFT_COLD_WHITELIST_LAT_SLACK},q${SHIFT_COLD_WHITELIST_Q_SLACK},jump${SHIFT_COLD_WHITELIST_MAX_SWITCH} seek_exit=${SHIFT_SEEK_EXIT_ANCHOR_CONFIDENCE}/${SHIFT_SEEK_EXIT_MIN_PROBE_OBS},stable${SHIFT_SEEK_EXIT_STABLE_WINDOWS}@r${SHIFT_SEEK_EXIT_STABLE_RADIUS}" | tee -a "${STATUS_LOG}"
echo "[$(date '+%F %T')] RETRY_KNOB max_retries_per_case=${MAX_RETRIES_PER_CASE}" | tee -a "${STATUS_LOG}"

# Heavy-first ordering: 6 rounds x (3 heavy + 1 small), 24 unique points.
JOBS=(
  "C01,1,8"  "C02,8,1"  "B08,5,5"  "A01,1,1"
  "C03,2,8"  "C04,8,2"  "B07,5,4"  "A02,1,2"
  "C05,3,8"  "C06,8,3"  "B06,4,5"  "A03,2,1"
  "C07,4,8"  "C08,8,4"  "B05,5,3"  "A04,2,2"
  "B04,3,5"  "B03,4,4"  "A07,3,3"  "A05,2,3"
  "B02,4,3"  "B01,3,4"  "A08,2,4"  "A06,3,2"
)
TOTAL_CASES="${#JOBS[@]}"
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_abcdef24_heavyfirst_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_abcdef24_${case_id}${retry_tag}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
  ...
  if env -u RAY_ADDRESS \
      ...
      TREAM_SHIFT_WARMUP_EXPLORE_ENABLE="${SHIFT_WARMUP_EXPLORE_ENABLE}" \
      TREAM_SHIFT_WARMUP_FORCE_DESCEND_LARGE="${SHIFT_WARMUP_FORCE_DESCEND_LARGE}" \
      TREAM_SHIFT_WARMUP_LARGE_CTX_THRESHOLD="${SHIFT_WARMUP_LARGE_CTX_THRESHOLD}" \
      TREAM_SHIFT_WARMUP_LARGE_INF_THRESHOLD="${SHIFT_WARMUP_LARGE_INF_THRESHOLD}" \
      TREAM_SHIFT_WARMUP_OSC_EXIT_ENABLE="${SHIFT_WARMUP_OSC_EXIT_ENABLE}" \
      TREAM_SHIFT_WARMUP_OSC_PATIENCE="${SHIFT_WARMUP_OSC_PATIENCE}" \
      TREAM_SHIFT_WARMUP_OSC_UNIQUE_MAX="${SHIFT_WARMUP_OSC_UNIQUE_MAX}" \
...
ray stop --force >> "${STATUS_LOG}" 2>&1 || true
start_ray_head "A" "${GPU_A}" "${RAY_PORT_A}" "${RAY_WORKER_MIN_A}" "${RAY_WORKER_MAX_A}" 56481 64241 61241 8861
start_ray_head "B" "${GPU_B}" "${RAY_PORT_B}" "${RAY_WORKER_MIN_B}" "${RAY_WORKER_MAX_B}" 56482 64242 61242 8862
start_ray_head "C" "${GPU_C}" "${RAY_PORT_C}" "${RAY_WORKER_MIN_C}" "${RAY_WORKER_MAX_C}" 56483 64243 61243 8863
start_ray_head "D" "${GPU_D}" "${RAY_PORT_D}" "${RAY_WORKER_MIN_D}" "${RAY_WORKER_MAX_D}" 56484 64244 61244 8864
start_ray_head "E" "${GPU_E}" "${RAY_PORT_E}" "${RAY_WORKER_MIN_E}" "${RAY_WORKER_MAX_E}" 56485 64245 61245 8865
start_ray_head "F" "${GPU_F}" "${RAY_PORT_F}" "${RAY_WORKER_MIN_F}" "${RAY_WORKER_MAX_F}" 56486 64246 61246 8866

lane_worker "A" "${GPU_A}" "${RAY_ADDR_A}" &
PID_A=$!
lane_worker "B" "${GPU_B}" "${RAY_ADDR_B}" &
PID_B=$!
lane_worker "C" "${GPU_C}" "${RAY_ADDR_C}" &
PID_C=$!
lane_worker "D" "${GPU_D}" "${RAY_ADDR_D}" &
PID_D=$!
lane_worker "E" "${GPU_E}" "${RAY_ADDR_E}" &
PID_E=$!
lane_worker "F" "${GPU_F}" "${RAY_ADDR_F}" &
PID_F=$!
...
TOTAL_FAIL=$((RC_A + RC_B + RC_C + RC_D + RC_E + RC_F))
if (( TOTAL_FAIL == 0 )); then
  echo "[$(date '+%F %T')] FINISH ${RUN_ID} all_success" | tee -a "${STATUS_LOG}"
else
  echo "[$(date '+%F %T')] FINISH ${RUN_ID} failures=${TOTAL_FAIL} (A=${RC_A}, B=${RC_B}, C=${RC_C}, D=${RC_D}, E=${RC_E}, F=${RC_F})" | tee -a "${STATUS_LOG}"
fi
