...
RUN_ID="abc24x3_trigpu_shift_taskpool_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_abc24x3_trigpu_shift_taskpool_run.txt"
...
# A queue (small workload)
CASES_A=(
  "1,1" "1,2" "2,1" "2,2"
  "2,3" "3,2" "3,3" "2,4"
)

# B queue (medium workload)
CASES_B=(
  "3,4" "4,3" "4,4" "3,5"
  "5,3" "4,5" "5,4" "5,5"
)

# C queue (large/cross workload)
CASES_C=(
  "1,8" "8,1" "2,8" "8,2"
  "3,8" "8,3" "4,8" "8,4"
)
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_abc24x3_taskpool_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_abc24x3_${case_id}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
