...
RUN_ID="abc24x4_quadgpu_shift_taskpool_jsdonly_heavyfirst_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_abc24x4_quadgpu_shift_taskpool_jsdonly_heavyfirst_run.txt"
...
echo "[$(date '+%F %T')] PLAN total_cases=24 dispatch=task_pool workers=4 pattern=3heavy+1small" | tee -a "${STATUS_LOG}"
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_abcd24_heavyfirst_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_abcd24_${case_id}${retry_tag}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
