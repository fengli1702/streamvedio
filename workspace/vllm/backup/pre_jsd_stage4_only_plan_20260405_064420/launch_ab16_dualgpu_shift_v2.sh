...
RAY_PORT_A=28121
RAY_PORT_B=28122
RAY_TEMP_A="/tmp/ray_ab16_a_${START_TS}"
RAY_TEMP_B="/tmp/ray_ab16_b_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_ab16_${queue}${tag}_${START_TS}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
  ...
  if env -u RAY_ADDRESS -u RAY_PORT -u RAY_GCS_SERVER_PORT \
      ...
      TREAM_SHIFT_WARMUP_HOLD_WINDOWS=4 \
      TREAM_SHIFT_FORCE_ADAPT_MIN_COUNT=6 \
      TREAM_SHIFT_COLD_RELAX_SAFETY=0 \
      TREAM_SHIFT_COLD_START_WINDOWS=8 \
      TREAM_SHIFT_COLD_PROBE_EVERY=2 \
      TREAM_SHIFT_COLD_WHITELIST_PROBE=1 \
      TREAM_SHIFT_COLD_WHITELIST_BUDGET=4 \
      TREAM_SHIFT_COLD_WHITELIST_LAT_SLACK=0.20 \
      TREAM_SHIFT_COLD_WHITELIST_Q_SLACK=0.60 \
...
queue_a() {
  ...
  run_one "A" "2" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 1 2 || failures=$((failures + 1))
  run_one "A" "3" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 2 2 || failures=$((failures + 1))
  run_one "A" "4" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 2 4 || failures=$((failures + 1))
  run_one "A" "5" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 3 2 || failures=$((failures + 1))
  run_one "A" "6" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 3 4 || failures=$((failures + 1))
  run_one "A" "7" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 4 2 || failures=$((failures + 1))
  run_one "A" "8" "${GPU_A}" "${RAY_PORT_A}" "${RAY_TEMP_A}" 4 4 || failures=$((failures + 1))
  return "${failures}"
}
...
queue_b() {
  ...
  run_one "B" "3" "${GPU_B}" "${RAY_PORT_B}" "${RAY_TEMP_B}" 3 8 || failures=$((failures + 1))
  run_one "B" "4" "${GPU_B}" "${RAY_PORT_B}" "${RAY_TEMP_B}" 4 8 || failures=$((failures + 1))
  run_one "B" "5" "${GPU_B}" "${RAY_PORT_B}" "${RAY_TEMP_B}" 4 6 || failures=$((failures + 1))
  run_one "B" "6" "${GPU_B}" "${RAY_PORT_B}" "${RAY_TEMP_B}" 6 6 || failures=$((failures + 1))
  run_one "B" "7" "${GPU_B}" "${RAY_PORT_B}" "${RAY_TEMP_B}" 6 8 || failures=$((failures + 1))
  run_one "B" "8" "${GPU_B}" "${RAY_PORT_B}" "${RAY_TEMP_B}" 8 8 || failures=$((failures + 1))
  return "${failures}"
}
