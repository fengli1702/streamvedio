...
RUN_ID="ab48x2_dualgpu_shift_taskpool_jsdonly_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_ab48x2_dualgpu_shift_taskpool_jsdonly_run.txt"
...
echo "[$(date '+%F %T')] SHIFT_KNOBS cold_windows=${SHIFT_COLD_START_WINDOWS} cold_max_windows=${SHIFT_COLD_START_MAX_WINDOWS} cold_probe_every=${SHIFT_COLD_PROBE_EVERY} cold_probe_eps_fast=${SHIFT_COLD_PROBE_EPS_FAST} cold_probe_eps_slow=${SHIFT_COLD_PROBE_EPS_SLOW} cold_avoid_two_cycle=${SHIFT_COLD_AVOID_TWO_CYCLE} cold_dir=${SHIFT_COLD_DIRECTIONAL_ENABLE} large_desc=${SHIFT_COLD_PREFER_LARGE_DESCEND}@ctx${SHIFT_COLD_LARGE_CTX_THRESHOLD},inf${SHIFT_COLD_LARGE_INF_THRESHOLD} small_asc=${SHIFT_COLD_PREFER_SMALL_ASCEND}@ctx${SHIFT_COLD_SMALL_CTX_THRESHOLD},inf${SHIFT_COLD_SMALL_INF_THRESHOLD} target=${SHIFT_COLD_TARGET_CTX},${SHIFT_COLD_TARGET_INF} band=${SHIFT_COLD_TARGET_CTX_BAND},${SHIFT_COLD_TARGET_INF_BAND} axis_lock=${SHIFT_COLD_SINGLE_AXIS_LOCK} early_exit=${SHIFT_COLD_EARLY_EXIT_ENABLE}/${SHIFT_COLD_TARGET_STABLE_WINDOWS} cold_patience_directions=${SHIFT_COLD_PATIENCE_DIRECTIONS} cold_axis_rotation=${SHIFT_COLD_AXIS_ROTATION} cold_relax_safety=${SHIFT_COLD_RELAX_SAFETY}@r${SHIFT_COLD_RELAX_RADIUS},q${SHIFT_COLD_RELAX_Q_SLACK},lat${SHIFT_COLD_RELAX_LAT_SLACK} cold_whitelist_budget=${SHIFT_COLD_WHITELIST_BUDGET} max_jump=${SHIFT_COLD_WHITELIST_MAX_SWITCH} min_count_for_trust=${SHIFT_MIN_COUNT_FOR_TRUST} adapt_probe_windows=${SHIFT_ADAPT_PROBE_WINDOWS} default_quality_min=${SHIFT_DEFAULT_QUALITY_MIN} scheduler_quality_min=${SCHEDULER_QUALITY_MIN} shock_jsd_only=${SHIFT_SHOCK_USE_JSD_ONLY}" | tee -a "${STATUS_LOG}"
...
start_ray_head() {
  ...
  local dash_port="$9"
  local temp_dir="/tmp/ray_ab48x2_taskpool_${lane}_${START_TS}"
...
run_one() {
  ...
  local run_name="doh_shift_ab48x2_${case_id}_${START_TS}_w${worker_lane}_c${ctx}_i${inf}_ib${IB}_tb${TB}"
...
for i in "${!CASES_C[@]}"; do
  tag="$(printf "%02d" "$((i + 1))")"
  JOBS+=("C${tag},${CASES_C[$i]}")
done
for i in "${!CASES_D[@]}"; do
  tag="$(printf "%02d" "$((i + 1))")"
  JOBS+=("D${tag},${CASES_D[$i]}")
done
for i in "${!CASES_E[@]}"; do
  tag="$(printf "%02d" "$((i + 1))")"
  JOBS+=("E${tag},${CASES_E[$i]}")
done
for i in "${!CASES_F[@]}"; do
  tag="$(printf "%02d" "$((i + 1))")"
  JOBS+=("F${tag},${CASES_F[$i]}")
done
