...
RUN_ID="abc24x3_trigpu_shift_taskpool_static_balanced_${START_TS}"
LOG_DIR="${TREAM_ROOT}/inference_logs"
MANIFEST="${LOG_DIR}/${RUN_ID}.tsv"
STATUS_LOG="${LOG_DIR}/${RUN_ID}.status.log"
LATEST_PTR="${LOG_DIR}/latest_abc24x3_trigpu_shift_taskpool_static_balanced_run.txt"
...
# - 8 cases per lane
# - C (large/cross) workload split 2/3/3
# - sum(ctx*inf) is exactly balanced across lanes (=111 each)
LANE_A_SPECS=(
  "A02,1,2" "A05,2,3" "A07,3,3" "A08,2,4"
  "B04,3,5" "B05,5,3" "C06,8,3" "C07,4,8"
)
LANE_B_SPECS=(
  "A01,1,1" "A06,3,2" "B01,3,4" "B03,4,4"
  "B06,4,5" "C01,1,8" "C03,2,8" "C08,8,4"
)
LANE_C_SPECS=(
  "A03,2,1" "A04,2,2" "B02,4,3" "B07,5,4"
  "B08,5,5" "C02,8,1" "C04,8,2" "C05,3,8"
)
